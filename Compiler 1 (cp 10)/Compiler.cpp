		...
		JackAnalyzer(string file){
			...
			try{
				if(file.length()>5 && file.substr(file.length()-5,5) == ".jack"){
					...
					/*
					ifstream stfiles(file);
					if(!(stfiles.is_open())){
						throw invalid_argument("File not found");
					}
					*/
					//files = new ifstream[1];
					//filesize = 1;
					//filecount = 1;
					//files[0] = move(stfiles);
			...
			for(const string foldfil : files){
				...
				cout << foldfil.substr(0,foldfil.length()-5) + ".xml" << endl;
				//JackTokenizer Tokeny(move(foldfilin));
				JackTokenizer Tokeny(foldfilin);
				/*
				Tokeny.advance();
				cout << to_string(static_cast<int>(Tokeny.tokenType)) << endl;
				Tokeny.advance();
				cout << to_string(static_cast<int>(Tokeny.tokenType)) << endl;
				cout << Tokeny.identifier << endl;
				Tokeny.advance();
				cout << to_string(static_cast<int>(Tokeny.tokenType)) << endl;
				*/
				...
				//cout << to_string(static_cast<int>(tokenTypeEnum::IDENTIFIER)) << endl;
			}
