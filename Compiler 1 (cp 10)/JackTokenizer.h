...
class JackTokenizer {
	public:
		JackTokenizer(ifstream &filein):file(filein){
			...
			//for(int i=0;i<20;i++){
				//tempchar = ' ';
				//temploc = tempind = content.find("//");
				//cout << to_string(tempchar != *"\n") << endl;
				/*
				while(tempchar != '\n' && tempind < content.length()){
					tempchar = content[++tempind];
					//cout << to_string(tempind);
				}
				*/
				for(;tempind<content.length() && content[tempind] != '\n';tempind++);
				//content = content.substr(0,temploc)+content.substr(tempind+1,content.length()-1);
			...
			while((temploc = tempind = content.find("/*"))!=-1){
			    /*
				while(!(tempchar == '*' && content[tempind+1] == '/') && tempind < content.length()){
					tempchar = content[++tempind];
				}
				*/
			...
			*/
			...
			//cout << to_string(cursor) << endl << "AtCur:" << content[cursor] << endl;
			//cout << static_cast<int>(content.at(0)) << endl << static_cast<int>(content.at(1)) << endl << static_cast<int>(content.at(2)) << endl;
		};
		/*
		bool hasMoreTokens(){};
		String tokenType(){};
	...
	private:
		...
		string content;
		char symbolChar[19] = {'}','{','(',')','.','[',']',',',';','+','-','*','/','&','|','<','>','=','~'};
		unsigned int symbolCharLen = 19;
		string keywordString[21] = {"class","method","function","constructor","int","boolean","char","void","var","static","field","let","do","if","else","while","return","true","false","null","this"};
		unsigned int keywordStringLen = 21;
		size_t cursor = 0;
