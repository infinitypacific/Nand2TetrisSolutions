		...
		JackCompiler(string file){
			...
			for(const string foldfil : files){
				...
				if(!(foldfilin.is_open())){
					...
					ofstream foldfilout(foldfil.substr(0,foldfil.length()-5) + ".vm");
					cout << "Init " << foldfil.substr(0,foldfil.length()-5) + ".vm" << endl;
					JackTokenizer Tokeny(foldfilin);
					//SymbolTable Tabley;
					VMWriter Writey(foldfilout);
					CompilationEngine Compiley(foldfilin,foldfilout,Tokeny,Writey);
