...
class CompilationEngine {
	public:
		...
		void CompileExpressionList(){
			...
			while(!(Tokenizer.tokenType == tokenTypeEnum::SYMBOL && Tokenizer.symbol == ')') && Tokenizer.hasMoreTokens){
				if(nedComma){
					if(Tokenizer.tokenType == tokenTypeEnum::SYMBOL && Tokenizer.symbol == ','){this->commitToken();Tokenizer.advance();}else{this->syntaxErr("Invalid ExpressionList!","Repeated expression with no sep; Floating "+tokenTypeTags[static_cast<short unsigned int>(Tokenizer.tokenType)]+"!");};
				}else{
					this->CompileExpression();
				}
				nedComma = !nedComma;
