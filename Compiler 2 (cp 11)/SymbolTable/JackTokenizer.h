...
#include <sys/types.h>
#include <stdexcept>
enum class keyWordEnum : unsigned short {CLASS,METHOD,FUNCTION,CONSTRUCTOR,INT,BOOLEAN,CHAR,VOID,VAR,STATIC,FIELD,LET,DO,IF,ELSE,WHILE,RETURN,TRUE,FALSE,J_NULL,THIS};
enum class tokenTypeEnum : unsigned short {KEYWORD,SYMBOL,IDENTIFIER,INT_CONST,STRING_CONST};
class JackTokenizer {
	public:
			...
			if(this->isNum(currentSelect)){
				...
				for(size_t i=0;i<keywordStringLen;i++){
					...
					if(currentSelect==currKeySel.at(0) && cursor+currKeySel.length()<content.length() && (this->isWhite(content[cursor+currKeySel.length()]) || this->isSymb(content[cursor+currKeySel.length()]))){
						...
						vldTok = true;
						for(short unsigned int j=1;j<currKeySel.length();j++){
	...
	private:
		...
		const short unsigned int symbolCharLen = 19;
		...
		const short unsigned int keywordStringLen = 21;
		size_t cursor = 0;
		...
		bool isSymb(char check){
			for(short unsigned int i=0;i<symbolCharLen;i++){
